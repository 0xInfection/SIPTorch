...
from mutators.urlencchar import urlEncodeStr
...
def escevr():
    ...
    '''
    log = logging.getLogger('escevr')
    log.info('Testing module: %s' % module_info['test'])
    msg = buildreq.makeRequest('REGISTER')
    mline, head, body = parseMsg(msg)
    # Tweak 1: modify the register header
    newmeth = urlEncodeStr('EGISTER', randomchoice=True, value=2)
    ...
    newct = '%s%s' % ('C', urlEncodeStr('ontact', value=1))
...
def run():
    ...
    if runPlugin(escevr(), minfo=module_info):
